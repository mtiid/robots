// =========================================================================
//  File: RobotClientMidiThru.ck
//  Takes midi input from an IAC bus and sends OSC to the robot server
//  -modified to pass midi thru by Ness Morris in November 2015
...
if(me.args()>1){
    if(!thru.open(Std.atoi(me.arg(1)))){
        if(!thru.open("Network Session 1")){
            <<<"Couldn't connect to network session...","">>>;
            me.exit();
            ;
        }
    }
...
<<<"Successfully connected to",thru.name() + "!">>>;

// connect to robot server
//oout.dest("chuckServer.local",50000);
oout.dest("localhost", 11235);
